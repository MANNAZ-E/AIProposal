...
/// disabled radio looks broken and, with nothing selected, refuses the first question.
/// </summary>
public class MaterialPresetTests
{
    ...
    [Fact]
    public void Everything_is_the_default_once_the_proposal_has_an_artifact()
    ...
    [Fact]
    public void Without_artifacts_the_default_falls_through_to_all_materials()
        => Assert.Equal(WorkingContextKind.SourceMaterial, MaterialSelection.DefaultPreset(
            [Document(DocumentType.MannazMaterialName)], []));

    /// <summary>An artifact that was never generated is not material, so it cannot make
    /// "Everything" the default any more than it is offered by the picker.</summary>
    [Fact]
