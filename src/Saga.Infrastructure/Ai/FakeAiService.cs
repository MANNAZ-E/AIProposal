...
/// <summary>
/// Stand-in used when Ai:UseFakeAi is set or AzureOpenAI:Endpoint is not configured, so the whole
/// artifact loop runs offline (and in tests) without spending tokens. Streams a canned markdown
/// response, and always reports non-zero tokens so metered dev runs produce realistic usage rows.
/// </summary>
public class FakeAiService : IAiService
{
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        var text = $"""
            *(Generated by the offline stand-in AI — set Ai:UseFakeAi to false and configure
            AzureOpenAI:Endpoint to use the real models.)*
